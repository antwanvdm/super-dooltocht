...
import { generateDigitaalQuestion } from '../../utils/digitaalData';

function DigitaalQuiz({ mathSettings, onSuccess, onFailure }) {
  ...
  useEffect(() => {
    const digitaalOps = {
      computerkennis: mathSettings?.enabledOperations?.computerkennis ?? true,
      veiligheid: mathSettings?.enabledOperations?.veiligheid ?? true,
      mediawijsheid: mathSettings?.enabledOperations?.mediawijsheid ?? true,
    };
    const q = generateDigitaalQuestion(digitaalOps);
    const allOptions = [
