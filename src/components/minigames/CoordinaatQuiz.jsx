...
import { generateCoordinaatQuestion, COLUMN_LABELS } from '../../utils/coordinatenData';

function CoordinaatGrid({ grid, gridCols, gridRows, highlightCell }) {
  return (
    <div className="inline-block">
      ...
      {grid.map((row, rowIdx) => (
        <div key={rowIdx} className="flex">
          ...
          {row.map((cell, colIdx) => {
            const isHighlight = highlightCell && highlightCell.col === colIdx && highlightCell.row === rowIdx;
            return (
              <div
                key={colIdx}
...
function CoordinaatQuiz({ mathSettings, onSuccess, onFailure }) {
  ...
  useEffect(() => {
    ...
    const q = generateCoordinaatQuestion(level);
    const allOptions = [
  ...
  return (
    <div className="text-center">
      <div className="mb-4 sm:mb-6">
        <p className="text-gray-600 text-sm sm:text-lg mb-2">Coördinaten</p>
        <div className="inline-flex flex-col items-center gap-3 bg-gradient-to-br from-teal-100 to-cyan-100 rounded-2xl px-4 sm:px-8 py-4 sm:py-6 border-2 border-teal-200">
          <CoordinaatGrid
            ...
            highlightCell={problem.highlightCell}
          />
        ...
        {options.map((option, index) => {
          ...
          return (
            <button
              ...
              className={`p-3 sm:p-4 rounded-xl font-medium text-sm sm:text-lg transition-all ${btnClass}`}
